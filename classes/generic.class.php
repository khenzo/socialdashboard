...
class Generic extends Connect {
	...
	public function guestOnly() {
		...
		if(!empty($_SESSION['jigowatt']['username'])) {
			$this->error =	"
							<div class='alert alert-error'>"._('You\'re already logged in. '). sprintf(_('Go <a href="%s">back</a> to the page you were viewing before this.'), 'javascript:history.go(-1)') ."</div>
							";
		}
	...
	public function get_gravatar( $email, $img = false, $s = 80, $d = 'mm', $r = 'g', $atts = array() ) {
		...
		$url .= "?s=$s&d=$d&r=$r";
		if ( $img ) {
			$url = '<img class="nav-user-photo" src="' . $url . '"';
